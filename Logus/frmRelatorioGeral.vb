            ...
            Select Case RelGeral_Tipo
                ...
                Case enRelGeral_Tipo.eContratoPAF_2011
                    ...
                    SqlText = "SELECT PAF.CD_CONTRATO_PAF," & _
                                     ...
                                     "NVL(CA.QT_KGS_OLD, PAF.QT_KGS) QT_KGS," & _
                                     "NVL(CA.DT_PRAZO_FIXACAO_OLD, PAF.DT_PRAZO_FIXACAO) DT_PRAZO_FIXACAO," & _
                                     "NVL(CA.DT_PRAZO_ENTREGA_OLD, PAF.DT_PRAZO_ENTREGA) DT_PRAZO_ENTREGA," & _
                              ...
                              " WHERE PAF.CD_CONTRATO_PAF = " & Filtro01 & _
                                " AND CA.CD_CONTRATO_PAF(+) = PAF.CD_CONTRATO_PAF" & _
                ...
                Case enRelGeral_Tipo.eNegociacao_ContratoPAF_Master
                    ...
                    SqlText = "SELECT PAF.CD_CONTRATO_PAF," & _
                                     ...
                                     "MFORN.NO_CIDADE NO_CIDADE_FORNECEDOR," & _
                                     "MFORN.CD_UF CD_UF_FORNECEDOR," & _
                                     "NVL(CAP.QT_KGS_OLD, PAF.QT_KGS) QT_KGS," & _
                                     "NVL(CAP.DT_PRAZO_ENTREGA_OLD, PAF.DT_PRAZO_ENTREGA) DT_PRAZO_ENTREGA," & _
                                     "NVL(CAN.VL_UNITARIO_OLD, NEG.VL_NEGOCIACAO) VL_NEGOCIACAO," & _
                              ...
                              " WHERE NEG.CD_CONTRATO_PAF = " & Filtro01 & _
                                ...
                                " AND CAP.CD_CONTRATO_PAF (+) = NEG.CD_CONTRATO_PAF" & _
                                " AND CAN.CD_CONTRATO_PAF (+) = NEG.CD_CONTRATO_PAF" & _
                                " AND CAN.SQ_NEGOCIACAO (+) = NEG.SQ_NEGOCIACAO" & _
                ...
                Case enRelGeral_Tipo.eDeclaracaoFixacaoPreco
                    ...
                    SqlText = "SELECT CF.CD_CONTRATO_PAF," & _
                                     ...
                                     "MFORN.NO_CIDADE NO_CIDADE_FORNECEDOR," & _
                                     "MFORN.CD_UF CD_UF_FORNECEDOR," & _
                                     "NVL(CAF.QT_KGS_OLD, CF.QT_KGS) QT_KGS," & _
                                     "NVL(CAF.VL_UNITARIO_OLD, CF.VL_UNITARIO) VL_UNITARIO," & _
                                     "TU.QT_FATOR," & _
                                     "N.DT_NEGOCIACAO," & _
                              ...
                              " FROM SOF.CONTRATO_PAF CP," & _
                                    ...
                                    "(SELECT *" & _
                                     " FROM SOF.CONTRATO_ADITIVO" & _
                                     " WHERE SQ_ADITIVO = 1" & _
                                       " AND SQ_CONTRATO_FIXO IS NOT NULL) CAF" & _
                              " WHERE CF.CD_CONTRATO_PAF = " & Filtro01 & _
                                " AND CF.SQ_NEGOCIACAO = " & Filtro02 & _
                                " AND CF.SQ_CONTRATO_FIXO = " & Filtro03 & _
                                " AND CAF.CD_CONTRATO_PAF (+) = CF.CD_CONTRATO_PAF" & _
                                " AND CAF.SQ_NEGOCIACAO (+) = CF.SQ_NEGOCIACAO" & _
                                " AND CAF.SQ_CONTRATO_FIXO (+) = CF.SQ_CONTRATO_FIXO" & _
                                " AND FIL.CD_FILIAL = CP.CD_FILIAL_ORIGEM" & _
                                ...
                                " AND CF.CD_TIPO_UNIDADE = TU.CD_TIPO_UNIDADE"
                    oData_01 = DBQuery(SqlText)
                ...
                Case enRelGeral_Tipo.eDeclaracaoFixacaoPreco_DiferencialBolsa, _
                    ...
                    SqlText = "SELECT CF.CD_CONTRATO_PAF," & _
                                     ...
                                     "F.IC_FISICA_JURIDICA IC_FISICO_JURIDICO," & _
                                     "0 VL_EXTENSO," & _
                                     "NVL(CAF.VL_TOTAL_OLD, CF.VL_TOTAL) VL_TOTAL," & _
                                     "CF.VL_TAXA_DOLAR" & _
                              " FROM SOF.CONTRATO_PAF CP," & _
                                    "SOF.FILIAL FIL," & _
                                    "SOF.FORNECEDOR F," & _
                              ...
                              " WHERE FIL.CD_FILIAL = CP.CD_FILIAL_ORIGEM" & _
                                ...
                                " AND CAF.CD_CONTRATO_PAF (+) = CF.CD_CONTRATO_PAF" & _
                                " AND CAF.SQ_NEGOCIACAO (+) = CF.SQ_NEGOCIACAO" & _
                                " AND CAF.SQ_CONTRATO_FIXO (+) = CF.SQ_CONTRATO_FIXO"
                    oData_02 = DBQuery(SqlText)
                ...
                Case enRelGeral_Tipo.eContrato_Aditivo
                    ...
                    SqlText = "SELECT ADV.*," & _
                              ...
                              " FROM SOF.CONTRATO_ADITIVO ADV," & _
                                    ...
                                    "SOF.MUNICIPIO MFIL," & _
                                    "SOF.FORNECEDOR F" & _
                              " WHERE ADV.SQ_ADITIVO = " & Filtro04 & _
                                " AND ADV.CD_CONTRATO_PAF = " & Filtro01

                    If Filtro02 > 0 Then
                        SqlText = SqlText & _
                                " AND ADV.SQ_NEGOCIACAO = " & Filtro02
                    Else
                        SqlText = SqlText & _
                                " AND ADV.SQ_NEGOCIACAO IS NULL "
                    End If
                    If Filtro03 > 0 Then
                        SqlText = SqlText & _
                                " AND ADV.SQ_CONTRATO_FIXO = " & Filtro03
                    Else
                        SqlText = SqlText & _
                                " AND ADV.SQ_CONTRATO_FIXO IS NULL "
                    End If
                    ...
                    For iCont = 0 To oData_02.Rows.Count - 1
                        ...
                        With oData_02.Rows(iCont)
                            ...
                            oRow.Item("CD_CONTRATO_PAF") = Trim(.Item("CD_CONTRATO_PAF"))
                            oRow.Item("DS_LOCAL_DATA") = "Local e Data: " & Trim(.Item("NO_CIDADE_FILIAL_EMISSAO")) & ", " & _
                                                                            DatePart(DateInterval.Day, .Item("DT_CRIACAO")) & " de " & VerMes(.Item("DT_CRIACAO")) & " de " & DatePart(DateInterval.Year, .Item("DT_CRIACAO"))
                            oRow.Item("NO_FILIAL") = .Item("NO_FILIAL")
                            oRow.Item("IC_JURIDICA") = .Item("IC_FISICO_JURIDICO")
                            oRow.Item("NO_CLIENTE") = .Item("NO_RAZAO_SOCIAL")
                            ...
                            End If
                            If Not CampoNulo(.Item("VL_UNITARIO")) And Not CampoNulo(.Item("SQ_CONTRATO_FIXO")) Then
                                bImprimir = True
